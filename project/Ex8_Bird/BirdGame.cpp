    ...
    : debugDraw(physicsScale)
{
    ...
    r.setWindowTitle("Bird Game");

    // I style ImGui to make for a nice scoreboard in the top left corner
...
void BirdGame::init()
{
    ...
    // Instead of making an incredibly big ground body, we just create a tiny circle which "follows" the bird
    auto groundObj = createGameObject();
    groundObj->name = "Ground";
    phys = groundObj->addComponent<PhysicsComponent>();
    phys->initCircle(b2_kinematicBody, 10 / physicsScale, {birdObj->getPosition().x / physicsScale, 120 / physicsScale}, 1);
    // Make the ground collider "follow" the bird by having the same horizontal velocity
    phys->setLinearVelocity({1, 0});
    groundObj->addComponent<DeadlyComponent>();

    // I do the same for "the ceiling"
    auto ceilObj = createGameObject();
    ceilObj->name = "Ceiling";
    phys = ceilObj->addComponent<PhysicsComponent>();
    phys->initCircle(b2_kinematicBody, 10 / physicsScale, {birdObj->getPosition().x / physicsScale, (windowSize.y + 9) / physicsScale}, 1);
    // Make the ceil collider "follow" the bird by having the same horizontal velocity
    phys->setLinearVelocity({1, 0});
    ceilObj->addComponent<DeadlyComponent>();
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        auto po = obj->addComponent<PhysicsComponent>();
        po->initBox(b2_staticBody, s / physicsScale, pos / physicsScale, 0);
        obj->addComponent<DeadlyComponent>();
    }
    ...
    for (int i = 0; i < length; i += 2)
    {
        ...
        float xOffset = xVariation * cos(i * curve * 0.2f);
        glm::vec2 pos{i * 300 + xOffset + 150, 250 + cos(i * curve) * heighVariation};
        obj->setPosition(pos);
        ...
        auto po = obj->addComponent<PhysicsComponent>();
        po->initCircle(b2_kinematicBody, s.x / physicsScale, pos / physicsScale, 0);
        po->setSensor(true);

        obj->addComponent<DestroyableComponent>()->setOnDestroy(onDestroy);
    }
...
void BirdGame::update(float time)
{
    if (cleanup)
    {
        ...
        auto it = std::remove_if(
            ...
            {
                auto comp = obj->getComponent<DestroyableComponent>();
                return comp != nullptr && comp->remove;
            });
...
void BirdGame::render()
{
    ...
    ImGui::SetNextWindowSize(ImVec2(100, 100), ImGuiSetCond_Always);
...
void BirdGame::updatePhysics()
{
    ...
    for (auto phys : physicsComponentLookup)
    {
        ...
        auto vel = phys.second->getLinearVelocity();
        gameObject->setRotation(glm::degrees(atan(vel.y / 3)));
    }
}
